...
def convert_flow(
    ...
    if not notebook_path:
        user = (
            os.environ.get("WORKSPACE_USER") or os.environ.get("USER_EMAIL") or "Shared"
        )
        base = f"/Users/{user}" if "@" in user else "/Shared"
    ...
    for idx, task in enumerate(ordered, start=10):
        full = next(
            (
                p
                for p in processors
                if p.get("type") == task.get("type")
                or p.get("name") == task.get("name")
            ),
            task,
        )
        ...
        if (
            "GetFile" in proc_type or "ListFile" in proc_type
        ) and "cloudFiles" not in code:
            al = json.loads(suggest_autoloader_options.func(json.dumps(props)))
            code = (
                f"# Suggested Auto Loader for {proc_type}\n{al['code']}\n# Tips:\n# - "
                + "\n# - ".join(al["tips"])
            )
    ...
    if also_import_notebook:
        ...
        if host and token:
            ...
            exists = (
                gs.status_code == 200 and gs.json().get("object_type") == "NOTEBOOK"
            )
            if not exists:
...
def orchestrate_intelligent_nifi_migration(
    ...
    try:
        ...
        logger.info(
            f"Analyzing NiFi architecture requirements for intelligent migration..."
        )
        ...
        logger.info(
            f"  - Total processors: {analysis['processor_analysis']['total_count']}"
        )
        logger.info(f"  - Complexity: {analysis['complexity_level']}")
        logger.info(f"  - Key features: {analysis['feature_flags']}")
        logger.info(
            f"  - Recommendation: {recommendation['recommendation']} (confidence: {recommendation['confidence']})"
        )
